...
/// Экран создания фиксированных пар для «Король корта» (is_paired).
/// Открывается из admin_tournament_detail_screen, когда турнир king_of_court с
/// фиксированными парами и пары ещё не созданы. После успеха возвращает true.
class AdminKocCreatePairsScreen extends StatefulWidget {
  final int tournamentId;
  final String tournamentName;

  const AdminKocCreatePairsScreen({
    super.key,
  ...
  @override
  State<AdminKocCreatePairsScreen> createState() =>
      _AdminKocCreatePairsScreenState();
}

class _AdminKocCreatePairsScreenState extends State<AdminKocCreatePairsScreen> {
  bool _loading = true;
  String? _error;
  bool _saving = false;
  ...
  Future<void> _load() async {
    ...
    try {
      final data =
          await context.read<AdminService>().getKocPairs(widget.tournamentId);
      if (!mounted) return;
      setState(() {
        _participants =
  ...
  Future<void> _submit() async {
    ...
    try {
      await context
          .read<AdminService>()
          .saveKocPairs(widget.tournamentId, pairs);
      if (!mounted) return;
