...
/// Экран создания фиксированных пар для турнира «Король Корта (Bali Format)».
/// Открывается из admin_tournament_detail_screen, когда турнир bali_koc и
/// пары ещё не созданы. После успешного сохранения возвращает true.
class AdminBaliCreatePairsScreen extends StatefulWidget {
  final int tournamentId;
  final String tournamentName;

  const AdminBaliCreatePairsScreen({
    super.key,
  ...
  @override
  State<AdminBaliCreatePairsScreen> createState() =>
      _AdminBaliCreatePairsScreenState();
}

class _AdminBaliCreatePairsScreenState
    extends State<AdminBaliCreatePairsScreen> {
  bool _loading = true;
  String? _error;
  bool _saving = false;

  // Зарегистрированные участники (доступные для выбора)
  List<Map<String, dynamic>> _participants = [];

  // Уже созданные пары (если есть) — readonly
  ...
  /// Выбор игроков для каждой пары: _selected[i] = [player1_id, player2_id]
  /// 0 = не выбран.
  ...
  Future<void> _load() async {
    ...
    try {
      final data =
          await context.read<AdminService>().getBaliKocPairs(widget.tournamentId);
      if (!mounted) return;
      setState(() {
        _participants =
  ...
  /// Авто-распределение: сильный + слабый.
  void _autoAssign() {
  ...
  /// Все занятые id для конкретного селекта (исключая текущий слот).
  ...
  Future<void> _submit() async {
    // Проверим что все слоты заполнены
    ...
    try {
      await context
          .read<AdminService>()
          .saveBaliKocPairs(widget.tournamentId, pairs);
      if (!mounted) return;
