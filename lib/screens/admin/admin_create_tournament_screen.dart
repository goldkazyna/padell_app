  ...
  Widget _typeSelector() {
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // Описание формата не помещается в четверть ширины экрана —
              // оно осталось в кнопке «О формате» под сеткой.
              Expanded(
                child: Text(title,
                    style: TextStyle(
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        // Четыре плитки в ряд: девять форматов занимают три ряда и целиком
        // помещаются на экран, без прокрутки.
  ...
  List<Widget> _formatChildren() {
    ...
    if (_type == 'americano_flex') {
      ...
      children.addAll([
        ...
        // Форматы, где сервер число кортов не читает (король корта, Bali,
        // американо, мексикано, классический, парный JPI): корты на игру
        // считаются от числа участников, значение влияет только на названия.
        if (_type != 'team' &&
            !(_type == 'just_padel_it' && !_flexIsPaired)) ...[
          const SizedBox(height: 4),
          Text(
            'У этого формата корты на игру считаются автоматически по числу '
            'участников — значение задаёт только количество названий кортов.',
            style: TextStyle(color: AppTheme.textDim, fontSize: 11),
          ),
        ],
        // Solo Just Padel It: посев строит сетку ровно кортов × 4, поэтому
        // при несовпадении кнопка старта турнира будет недоступна.
        if (_type == 'just_padel_it' && !_flexIsPaired) ...[
          const SizedBox(height: 4),
          Text(
            'Кнопка посева появится, только когда зарегистрированных будет '
            'ровно кортов × 4.',
            style: TextStyle(color: AppTheme.textDim, fontSize: 11),
          ),
        ],
